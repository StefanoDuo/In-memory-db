...
class CreateTable(unittest.TestCase):
   ...
   def test_create_table(self):
      output = [str(sql_interpreter.database.Table(names, types)) for names, types in zip(self.column_names, self.column_types)]
      self.assertSequenceEqual(output, self.create_result)

   def test_insert_into_table(self):
      empty_tables = [sql_interpreter.database.Table(names, types) for names, types in zip(self.column_names, self.column_types)]
      output = []
   ...
   def test_cartesian_product(self):
      tables = [sql_interpreter.database.Table(names, types) for names, types in zip(self.column_names, self.column_types)]
      for table, rows in zip(tables, self.values):
         for row in rows:
            table.insert_row(row)
      cartesian_product = sql_interpreter.database.Table.cartesian_product(tables)
